...
RUN dnf update -y && dnf install -y aria2 jq curl wget git-lfs git gcc make ranger nvim opentofu openssl-devel perl-IPC-Cmd perl-FindBin perl-devel openssl tcpdump btop cmake tldr && dnf clean all
...
RUN . "$CARGO_HOME/env" && cargo install zellij git-delta fd-find sd procs ripgrep bat hyperfine atuin zoxide exa rustscan starship@1.20.0 du-dust gping
