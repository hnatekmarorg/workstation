...
RUN dnf update -y && dnf install -y iputils aria2 jq curl wget git-lfs git gcc make ranger nvim opentofu openssl-devel perl-Digest-SHA-1 perl-IPC-Cmd perl-FindBin perl-devel openssl tcpdump btop cmake tldr && dnf clean all
